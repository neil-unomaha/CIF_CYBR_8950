...
As a result of this project, the team was able to create a new endpoint within the existing CIF infrastructure. It was designed to assist in pulling feeds for Palo Alto firewalls to request and ingest in a recognized format. The endpoint can listen to requests for the latest threat feeds. The metric analysis was performed on a 2.5-hour packet capture without the endpoint implementation. After manual analysis, it was found within the packet capture, the endpoint could help eliminate 50-60% of malicious traffic over the network.
